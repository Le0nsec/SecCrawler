...
func (bot TgBot) Send(crawlerResult [][]string, description string) error {
	...
	data := fmt.Sprintf("{\"chat_id\":\"%s\",\"text\":\"%s\",\"parse_mode\":\"Markdown\"}", Cfg.Bot.TgBot.ChatId, title+msg)

	req, err := http.NewRequest("POST", fmt.Sprintf("https://api.telegram.org/bot%s/sendMessage", Cfg.Bot.TgBot.Token), strings.NewReader(data))
	if err != nil {
		return err
	}
